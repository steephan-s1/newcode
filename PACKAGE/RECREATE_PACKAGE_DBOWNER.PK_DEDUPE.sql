CREATE PACKAGE DBOWNER.PK_DEDUPE
  CREATE OR REPLACE EDITIONABLE PACKAGE "DBOWNER"."PK_DEDUPE" AS
...
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DBOWNER"."PK_DEDUPE" AS
...
BEGIN
  ...
  SELECT count(*) INTO v_transfers  FROM tbl_sources s 
	...
	ELSE
		pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'locking transfers out using tbl_dedupe_control', v_pos, TRUE);
	...
	FOR rec IN cur LOOP
		...
		-- COMMIT;
            IF l_count_updates > 10000 THEN
                COMMIT;
                l_count_updates := 0;
                pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'Completed COMMITING updates ', v_pos, TRUE);
            ELSE
	...
	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'unlocking transfers using tbl_dedupe_control', v_pos, TRUE);
