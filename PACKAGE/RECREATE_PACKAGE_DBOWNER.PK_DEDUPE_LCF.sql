CREATE PACKAGE DBOWNER.PK_DEDUPE_LCF
  CREATE OR REPLACE EDITIONABLE PACKAGE "DBOWNER"."PK_DEDUPE_LCF" AS
...
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DBOWNER"."PK_DEDUPE_LCF" AS
...
BEGIN
	...
	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'checking for transfers running', v_pos, TRUE);
  ...
  SELECT count(*) INTO v_transfers  FROM tbl_sources s 
	...
	ELSE
		pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'locking transfers out using tbl_dedupe_control', v_pos, TRUE);
		--UPDATE tbl_dedupe_control SET dedupe_running = 'Y';
		--COMMIT;
	END IF;


	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'just about to truncate tbl_deduping', v_pos, TRUE);
	EXECUTE IMMEDIATE 'TRUNCATE TABLE tbl_deduping_lcf';

	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'just about to insert into tbl_deduping', v_pos, TRUE);
	INSERT /*+ append */ INTO tbl_deduping_lcf
	...
	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'commencing dedupe update', v_pos, TRUE);
	FOR rec IN cur LOOP
		...
		-- COMMIT;
            IF l_count_updates > 10000 THEN
                COMMIT;
                l_count_updates := 0;
                pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'Completed COMMITING updates ', v_pos, TRUE);
            ELSE
                l_count_updates := l_count_updates + 1;
            END IF;

	END LOOP;
	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'end of dedupe update', v_pos, TRUE);

	pk_main.log_time(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 'unlocking transfers using tbl_dedupe_control', v_pos, TRUE);
...
WHEN e_transfers_running THEN
	...
	p_success := 'N';
	pk_main.log_error(NULL, 'pk_dedupe_lcf', 'dedupe_parties', 0, 'there are transfer processes already running', v_pos);


WHEN OTHERS THEN

	p_success := 'N';
	pk_main.log_error(NULL, 'pk_dedupe_lcf', 'dedupe_parties', SQLCODE, SQLERRM, v_pos);
...
WHEN e_two_duplicate_keys THEN
	...
	pk_main.log_error(p_source, 'pk_dedupe_lcf', 'assign_party_dedupe_id', 0, v_error_message, v_pos);

WHEN OTHERS THEN

	v_error_message := 'Error (' || p_orig_party_ref || '): ' || SQLERRM;
	pk_main.log_error(p_source, 'pk_dedupe_lcf', 'assign_party_dedupe_id', SQLCODE, v_error_message, v_pos);
...
WHEN e_two_duplicate_keys THEN
	...
	pk_main.log_error(p_source, 'pk_dedupe_lcf', 'get_item_dedupe_id', 0, v_error_message, v_pos);
	RAISE e_two_duplicate_keys;

WHEN OTHERS THEN

	v_error_message := 'Error (' || p_orig_system_ref || '): ' || SQLERRM;
	pk_main.log_error(p_source, 'pk_dedupe_lcf', 'get_item_dedupe_id', SQLCODE, v_error_message, v_pos);
...
WHEN OTHERS THEN
	...
	pk_main.log_error('', 'pk_dedupe_lcf', 'create_dedupe_table', SQLCODE, SQLERRM, v_pos);
...
WHEN OTHERS THEN
	...
	pk_main.log_error('', 'pk_dedupe_lcf', 'apply_match_key', SQLCODE, SQLERRM, v_pos);
