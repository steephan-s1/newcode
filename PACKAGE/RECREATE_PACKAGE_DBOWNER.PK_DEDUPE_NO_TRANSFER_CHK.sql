CREATE PACKAGE DBOWNER.PK_DEDUPE_NO_TRANSFER_CHK
  CREATE OR REPLACE EDITIONABLE PACKAGE "DBOWNER"."PK_DEDUPE_NO_TRANSFER_CHK" AS
...
END pk_dedupe_NO_TRANSFER_CHK;
 
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "DBOWNER"."PK_DEDUPE_NO_TRANSFER_CHK" AS
/****************************************************************
File Name    : pk_dedupe_NO_TRANSFER_CHK.spc
Author        : Mike Ranken
Description    : Run DeDupe without checking for Transfer RUN
Audit        :
    Version        Date    User            Description
        2    14-JUL-2014    Mike Ranken      Initial Version
        2    22-JUL-2015    Ganesh Shekar    JIRA Added Parallelism and enhancements
****************************************************************/
...
--// GCS Changes: No need of DISTINCT
    -- SELECT DISTINCT orig_system, dedupe_type, orig_party_ref, party_name, email, record_status, dedupe_id, capri_update_date
    SELECT /*+ PARALLEL(d, 4) */ orig_system, dedupe_type, orig_party_ref, party_name, email, record_status, dedupe_id, capri_update_date
	FROM tbl_deduping
	WHERE record_status IN ('I','U')
	ORDER BY capri_update_date;

v_pos 	  	 			NUMBER(1) := 0;
v_transfers 			NUMBER(2);
...
BEGIN
	...
	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'checking for transfers running', v_pos, TRUE);
  ...
  AND s.source in (
                ...
                and parties_last_update >= relationships_start                        
                );
  ...
  IF v_transfers > 0 THEN
	...
	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'just about to truncate tbl_deduping', v_pos, TRUE);
	EXECUTE IMMEDIATE 'TRUNCATE TABLE tbl_deduping';

	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'just about to insert into tbl_deduping', v_pos, TRUE);
    INSERT /*+ append */ INTO tbl_deduping
	...
	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'commencing dedupe update', v_pos, TRUE);
	FOR rec IN cur LOOP
        ...
        -- COMMIT;
            ...
            END IF;
	END LOOP;
	pk_main.log_time(NULL, 'pk_dedupe', 'dedupe_parties', 'end of dedupe update', v_pos, TRUE);
...
WHEN e_transfers_running THEN
	...
	p_success := 'N';
	pk_main.log_error(NULL, 'pk_dedupe', 'dedupe_parties', 0, 'there are transfer processes already running', v_pos);


WHEN OTHERS THEN

	p_success := 'N';
	pk_main.log_error(NULL, 'pk_dedupe', 'dedupe_parties', SQLCODE, SQLERRM, v_pos);
...
WHEN e_two_duplicate_keys THEN
	...
	pk_main.log_error(p_source, 'pk_dedupe', 'assign_party_dedupe_id', 0, v_error_message, v_pos);

WHEN OTHERS THEN

	v_error_message := 'Error (' || p_orig_party_ref || '): ' || SQLERRM;
	pk_main.log_error(p_source, 'pk_dedupe', 'assign_party_dedupe_id', SQLCODE, v_error_message, v_pos);
...
WHEN e_two_duplicate_keys THEN
	...
	pk_main.log_error(p_source, 'pk_dedupe', 'get_item_dedupe_id', 0, v_error_message, v_pos);
	RAISE e_two_duplicate_keys;

WHEN OTHERS THEN

	v_error_message := 'Error (' || p_orig_system_ref || '): ' || SQLERRM;
	pk_main.log_error(p_source, 'pk_dedupe', 'get_item_dedupe_id', SQLCODE, v_error_message, v_pos);
...
WHEN OTHERS THEN
	...
	pk_main.log_error('', 'pk_dedupe', 'create_dedupe_table', SQLCODE, SQLERRM, v_pos);
...
WHEN OTHERS THEN
	...
	pk_main.log_error('', 'pk_dedupe', 'apply_match_key', SQLCODE, SQLERRM, v_pos);
